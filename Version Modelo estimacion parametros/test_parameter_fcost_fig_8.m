...
p3=[5.38983788162671,0.580263090372408,0.856119020389810,0.191153343167985,0.0397955538854417,2225.62638937998,2.77197739721344]
%Resultados 4 0,9
...
%     p=p_nom;
    Mcp_c=p_nom(1);Phi_ef=  p_nom(2);Eff_area=p_nom(3);Q_loss=  p_nom(4);x0_ini(2)=p_nom(5);
    ...
    dist_model_0{i}=data_n_rs.Data(:,:);
       n=length(T_vector);
        NMSE_A_0(i)=(1/n)*sum(((dist_exp{i}(m_s:end-m_f,6)-dist_model_0{i}(m_s:end-m_f,1))./max(dist_exp{i}(m_s:end-m_f,6))).^2)
    NMSE_Tout_0(i)=(1/n)*sum(((dist_exp{i}(m_s:end-m_f,4)-dist_model_0{i}(m_s:end-m_f,8))./max(dist_exp{i}(m_s:end-m_f,4))).^2)
    
num_A=sum((dist_exp{i}(m_s:end-m_f,6)-dist_model_0{i}(m_s:end-m_f,1)).^2);
den_A=sum((dist_exp{i}(m_s:end-m_f,6)-mean(dist_exp{i}(m_s:end-m_f,6))).^2);

num_Tout=sum((dist_exp{i}(m_s:end-m_f,4)-dist_model_0{i}(m_s:end-m_f,8)).^2);
den_Tout=sum((dist_exp{i}(m_s:end-m_f,4)-mean(dist_exp{i}(m_s:end-m_f,4))).^2);


    R2_adj_A_0(i)=1-(num_A/den_A)*(n-1)/(n-5-1)
    R2_adj_Tout_0(i)=1-(num_Tout/den_Tout)*(n-1)/(n-5-1)
    
end

%% Modelo Modificado
% Bucle for para correr las destilaciones
for i=3%1:length(dist_index)
...
%  v_t=[9 10 11 12];
 ...
 for i=3%1:4
    ...
    subplot(3,1,1)  % 1 2 3 4
    
    plot(destilaciones3{i}(1:m_f,2)/60,destilaciones3{1,i}(1:m_f,6),'--','Linewidth',lw1,'Color','k'); hold on
    plot(dist_exp{i}(1:m_f,2)/60,dist_model_0{1,i}(1:m_f,1),'Linewidth',lw2,'Color','r'); hold on
    plot(dist_exp{i}(1:m_f,2)/60,dist_model{1,i}(1:m_f,1),'Linewidth',lw2,'Color','b')
...
%     title(titles{i})
    ...
    subplot(3,1,3) % 5 6 7 8
        plot(destilaciones3{i}(1:m_f,2)/60,destilaciones3{1,i}(1:m_f,7),'--','Linewidth',lw1,'Color','k'); hold on 
        plot(dist_exp{i}(1:m_f,2)/60,dist_model_0{1,i}(1:m_f,7),'Linewidth',lw2,'Color','r'); hold on
        plot(dist_exp{i}(1:m_f,2)/60,dist_model{1,i}(1:m_f,7),'Linewidth',lw2,'Color','b')
        

    ylabel('{\it F_w} (mL/min)')
    xlabel('Time, h')
    grid on
    subplot(3,1,2) % 5 9 10 11 12
    plot(destilaciones3{i}(1:m_f,2)/60,destilaciones3{1,i}(1:m_f,4),'--','Linewidth',lw1,'Color','k') ; hold on
    plot(dist_exp{i}(1:m_f,2)/60,dist_model_0{1,i}(1:m_f,8),'Linewidth',lw2,'Color','r'); hold on
    plot(dist_exp{i}(1:m_f,2)/60,dist_model{1,i}(1:m_f,8),'Linewidth',lw2,'Color','b')
    
    
%     legend('Exp. Data','Previous Model','Modified Model')
      ylabel('{\it T_w_,_o_u_t}  (ºC)')
%     legend('Location','best')
    xlabel('Time, h')
    grid on
    
    % Experimental.   (3: Tin)  (4: Tout) (5:T
