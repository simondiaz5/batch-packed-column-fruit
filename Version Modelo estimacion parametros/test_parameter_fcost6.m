function test_parameter_fcost5(p)
...
lw2=1;
 ...
 for i=1:4
    ...
    plot(dist_exp{i}(1:end,2)/60,dist_model{1,i}(1:end,1),'Linewidth',lw2)
    title(titles{i})
    legend('exp','model')
    ylabel('Aº_d (%v/v)')
    xlabel('Time (h)')
    ...
    plot(dist_exp{i}(1:end,2)/60,dist_model{1,i}(1:end,8),'Linewidth',lw2)
    legend('exp','model')
    ylabel('T_o_u_t (ºC)')
    xlabel('Time, h')
    grid on
