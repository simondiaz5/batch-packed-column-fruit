% function test_parameter_fcost3(p,index_140 ,index_200 ,index_250)
...
for i=1:2
    ...
    ylabel('{\it A_d} (%v/v)')
    xlabel('Time (h)')
    title({'{\it Q_c} = 140 W (Calibration)'})
    grid on
...
%                         title('Ethanol concentration, %v/v')
    title({'{\it Q_c} = 250 W (Calibration)'})
    ylabel('{\it A_d} (%v/v)')
    xlabel('Time (h)')
                        grid on
