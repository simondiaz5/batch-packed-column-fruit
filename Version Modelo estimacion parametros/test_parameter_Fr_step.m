function test_parameter_Fr_step(p)
...
% p0=[5.582810105642534,0.853310648588036,0.305660802905230,30,0.043142717739935];
...
% Q_loss=15; % watt
Mcp_c=   p0(1);  % Cp*m metal
Phi_ef=  p0(2);  % Factor eficiencia area transf. masa
Eff_area=p0(3);  % Factor eficiencia area transf. calor
Q_loss=  p0(4);  % watt

x0_ini(2)=p(1); % Valor inicial etanol boiler
% Entrada constante
Qb=600;        % Potencia calefacciòn boiler W
...
for i=1:2
    ...
    dist_exp(i)=destilaciones2((i)); % aux. matriz de datos exp.
...
%                         grid on
%                     end
                     sgtitle('Model Validation - Coolant Flow Steps')
                     grid on
                     set(gcf,'color','white')
